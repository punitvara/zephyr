...
then resume those devices after waking up from deep sleep, UART device supposed
to keep output via console.
...
#define TICKLESS_IDLE (SYS_PM_DEEP_SLEEP+1)
#define DURATION (RTC_ALARM_SECOND*2)
#define SLEEP_MS 200
#define STATE_INVALID  4

extern void test_sysdev_state(int);
...
static void wakeup_config(uint32_t value)
{
	...
	if (rtc_wakeup) {
		...
		zassert_false(counter_set_alarm(aon_dev, NULL, value, NULL), NULL);
	}
}
...
{DEVICE_PM_SUSPEND_STATE, SYS_POWER_STATE_MAX, "TICKLESS_IDLE"}
};
...
void enter_sys_power_state(int index)
{
	...
	if (sys_state[index].soc_state != SYS_POWER_STATE_MAX) {
		...
		_sys_soc_set_power_state(sys_state[index].soc_state);
...
int _sys_soc_suspend(int32_t ticks)
{
	...
	case TICKLESS_IDLE:
		TC_PRINT("enter TICKLESS_IDLE\n");
		enter_sys_power_state(TICKLESS_IDLE);
		pmstate = SYS_PM_ACTIVE_STATE;
...
/**
 * @brief power state transition active->tickless idle (halt)->acive
 * @return N/A
 */
void test_tickless(void)
{
	pmstate = TICKLESS_IDLE;
	k_sleep(SLEEP_MS);
	exit_sys_power_state(TICKLESS_IDLE);
	TC_PRINT("exit TICKLESS_IDLE\n");
	/*test functionality of resumed device*/
	if (hook_dev_func) {
		hook_dev_func();
	}
